	...
	<div :class="classes">
		...
		<label :for="'title-' + task.id" :aria-label="task.title" class="title">
			<input type="text" readonly :value="task.title" :id="'title-' + task.id" name="title"
				placeholder="Input title" style="background-color: red" />
			/>
		</label>
	...
	setup(props, { emit }) {
		...
		return {
			classes: computed(() => ({
				...
				'list-item TASK_CUSTOM': props.task.state === 'TASK_CUSTOM',
			})),
			/**
			...
			/**
			 * Event handler for custom tasks
			 */
			customTask() {
				emit('custom-task', props.task.id);
			},
		};
	},
};
